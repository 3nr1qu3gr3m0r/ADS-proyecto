...
| Capa | Tecnología |
|------|-----------|
| Frontend | React 18 · TypeScript 5.6 · Vite 6 · Tailwind CSS 3.4 · SVG |
| Backend | Python 3.11 · FastAPI · Uvicorn · Pydantic |
| Protocolo | HTTP REST · JSON |
| CI/CD | GitHub Actions |
| Deploy | Railway (PaaS) con nixpacks |
...
| `typecheck` | `tsc --noEmit` | Verificación de tipos solamente |
...
- `test_motor.py`: 15 tests (bitpair helpers, NOT, AND, OR, UI, contradicción B)
- `test_simulador.py`: Tests de inicialización, navegación, pausa/reanudación, aislamiento
...
El pipeline se ejecuta en cada push y PR a las ramas `main` y `develop`:

| Job | Acción |
|-----|--------|
| `lint` | Typecheck del frontend (`npm run typecheck`) |
| `test` | Build de producción del frontend (`npm run build`) |
| `python-tests` | Tests del backend (`pytest`) |
...
Railway es una plataforma PaaS que permite desplegar aplicaciones desde un repositorio de GitHub. Se crean **dos servicios separados**: uno para el backend y otro para el frontend.
...
git init
git add .
git commit -m "feat: EPiC Playground"
...
5. Click en **Deploy**.
6. Copiar la URL pública del backend (ej: `https://epic-backend-prod.up.railway.app`).
...
│   │   ├── contratos.py        ← Interfaces abstractas (IMotorCalculo, ISimulacion)
...
│   ├── railway.toml            ← Configuración de despliegue Railway
│   ├── package.json            ← epic-playground-editor v1.0.0
│   ├── vite.config.ts
│   ├── tsconfig.json
│   ├── tailwind.config.js
│   ├── postcss.config.js
...
│   │
│   └── src/
│       ├── main.tsx            ← ReactDOM root render (StrictMode)
│       ├── index.css           ← Variables CSS, animaciones, tema oscuro
│       ├── App.tsx             ← Estado global, routing de tabs, simulación
│       │
│       ├── compartido/         ← Todos los subequipos
│       │   ├── tipos.ts        ← Interfaces TypeScript (Nodo, Arista, Red, etc.)
...
│       │
│       └── visualizador/       ← Todos
│           ├── ControlesSimulacion.tsx ← Play/Pause/Step Forward/Step Back
│           └── PanelResultados.tsx     ← Regla disparada, nodo que cambió
│
└── Documentacion/
    ├── Presentacion_Simulador_Visualizador.pdf
    └── documentacion/
```
