...
// usando fetch nativo: calcularRed, iniciarSimulacion, obtenerSimulacion,
// pausar/reanudar/avanzar/retroceder paso, con manejo de errores por código HTTP
// y URL base configurable desde la variable de entorno VITE_API_URL.
// ======== FIN DEL PROMPT ========

/**
 * api.ts — Cliente HTTP para Motor y Simulador.
 */
...
export async function obtenerSimulacion(id: string): Promise<EstadoSimulacion> {
  const res = await fetch(`${BASE}/api/v1/simulaciones/${id}`);
  if (!res.ok) throw new Error(`${res.status}`);
  return res.json() as Promise<EstadoSimulacion>;
}

export async function pausarSimulacion(id: string): Promise<void> {
  await _put(`/api/v1/simulaciones/${id}/pausar`);
}

export async function reanudarSimulacion(id: string): Promise<void> {
  await _put(`/api/v1/simulaciones/${id}/reanudar`);
}
